...
def shiftUp(BH,i):
    ...
    while i // 2 > 0:
        aux2=(i // 2)-1
        pad=linkedlist.access(BH.bheaplist, aux2)
        aux=linkedlist.access(BH.bheaplist, i)
        if aux > pad:
            linkedlist.update(BH.bheaplist, aux, aux2)
            linkedlist.update(BH.bheaplist, pad, i)
        i = i // 2
...
def shiftDown(BH,i,currentsize=None):
    ...
    while (i * 2) <= currentsize:
        ...
        nodoi = linkedlist.access(BH.bheaplist, i)
        if mc>nodoi:
            linkedlist.update(BH.bheaplist,nodoi,mc)
        i = mc
